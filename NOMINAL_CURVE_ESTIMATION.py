...
SINGLE_YM = "2019-12"   # yyyy-mm — resolved automatically to last trading day
...
def plot_panel_rmse(results: pd.DataFrame):
    ...
    ax.plot(dates, results["rmse_yield_bp_actual"],
            color="#0072B2", lw=1.8, ls="-",  zorder=3,
            label="Actual fit")
...
def plot_panel_rmse_bins(bin_df: pd.DataFrame):
    ...
    for ax, label in zip(axes, bin_labels):
        ...
        ax.plot(dates, sub["rmse_actual"],
                color="#0072B2", lw=1.6, ls="-",  zorder=3,
                label="Actual fit")
        ax.plot(dates, sub["rmse_ext"],
                color="#222222", lw=1.6, ls="--", zorder=3,
                label="Extended fit")
...
# for three maturity sets, exports to Excel, and plots the mean time series.
#
# Sets:
#   0-5y  — bonds with TTM in (0, 5] years
#   0-10y — bonds with TTM in (0, 10] years
...
def plot_bid_ask_means(summary: pd.DataFrame):
    ...
    yield spread for the three maturity sets.
    """
    dates = pd.to_datetime(summary["date"])

    # Three distinct colours from the existing palette family
    colors = {
        "0_5y":  "#1A5EA8",   # navy
        "0_10y": "#8B1A1A",   # claret
        "all":   "#3A3A3A",   # charcoal
...
bid_ask_summary = compute_bid_ask_summary(df_SGBs)
...
def plot_bonds_outstanding():
    ...
    ax1.set_ylim(0, df["ttm_years"].max() * 1.05)
    ax1.margins(x=0)
    ...
    #ax2.set_xlabel("Date")
    ax2.set_ylim(0, count_df["n_bonds"].max() + 1)
